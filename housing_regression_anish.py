...
# Realistic price formula (target in $100k)
...
print(f"\n  Dataset   : California-style Housing Dataset (synthetic, n=2000)")
...
# Plot 3: Scatter - Price vs Top Features
...
for ax, feat in zip(axes, ['MedInc', 'AveRooms', 'HouseAge', 'AveOccup']):
    ...
    ax.set_xlabel(feat, fontsize=11); ax.set_ylabel('Price ($100k)')
...
for col in sc_df.columns:
    print(f"  {col:<12}  {sc_df[col].mean():>8.4f}  {sc_df[col].std():>8.4f}  {sc_df[col].min():>8.4f}  {sc_df[col].max():>8.4f}")
...
for name, model in models.items():
    ...
    results[name] = {
        ...
        'CV_R2':round(cv,4),'model':model,'preds':y_pred
    }
...
for name, r in results.items():
    marker = " <-- BEST" if name == max(results, key=lambda k: results[k]['R2']) else ""
    print(f"  {name:<26} {r['MSE']:>7.4f} {r['RMSE']:>7.4f} {r['MAE']:>7.4f} {r['R2']:>7.4f} {r['CV_R2']:>7.4f}{marker}")
...
print(SUB)
rf_imp = pd.Series(results['Random Forest']['model'].feature_importances_, index=X.columns).sort_values(ascending=False)
...
ax.set_ylabel('R2 Score'); ax.set_ylim(0, 1.12)
...
ax.set_title(f'Actual vs Predicted - {best_name}\nR2 = {best["R2"]}', fontsize=13, fontweight='bold')
ax.legend()
plt.tight_layout()
...
axes[0].set_xlabel('Predicted Price ($100k)'); axes[0].set_ylabel('Residuals')
...
# ======================================================
print(f"\n{DIVIDER}")
print("  STEP 8: SAMPLE PREDICTIONS (Best Model)")
print(DIVIDER)
print(f"\n  Model: {best_name}\n")
print(f"  {'Sample':<8} {'Actual ($100k)':>16} {'Predicted ($100k)':>18} {'Error ($100k)':>14} {'Error ($)':>14}")
print(f"  {'-'*72}")
...
# ======================================================
# 9. GOODNESS-OF-FIT SUMMARY
# ======================================================
print(f"\n{DIVIDER}")
print("  STEP 9: GOODNESS-OF-FIT MEASURES EXPLAINED")
print(DIVIDER)
print(f"""
  ...
  RMSE    Root Mean Squared Error       0.00  Avg error (same units as target)
  MAE     Mean Absolute Error           0.00  Avg absolute error (robust)
  CV_R2   Cross-Validated R2            1.00  Generalisation performance (5-fold)

  Results Summary:
  {'Model':<26} {'R2':>7} {'Grade'}
  {SUB}""")
for name, r in sorted(results.items(), key=lambda x: -x[1]['R2']):
    grade = 'Excellent' if r['R2']>=0.85 else 'Good' if r['R2']>=0.75 else 'Fair' if r['R2']>=0.60 else 'Poor'
...
print(f"""
  ...
  Models Applied  :
    ...
    6. Gradient Boosting       - Boosted ensemble, state-of-art

  Key Findings    :
    * MedInc is the STRONGEST predictor (highest correlation + RF importance)
    * AveRooms and Latitude also significantly influence prices
    * Ensemble models outperform linear models by a large margin
    * Best Model : {best_name}
        R2   = {best['R2']}  -> model explains {best['R2']*100:.1f}% of price variance
        RMSE = {best['RMSE']} ($100k) -> avg error of +/- ${best['RMSE']*100000:,.0f}
        MAE  = {best['MAE']} ($100k) -> median error of +/- ${best['MAE']*100000:,.0f}

  Output Files    :
    housing_regression.py           Python source code
    plot1_distributions.png         Feature histograms
    plot2_heatmap.png               Correlation heatmap
    plot3_scatter.png               Price vs top features
    plot4_r2_comparison.png         R2 across all models
    plot5_actual_vs_pred.png        Actual vs Predicted
    plot6_residuals.png             Residual analysis
    plot7_rmse_comparison.png       RMSE across all models
